...
export const useWishlist = () => {
  ...
  const loadWishlist = async () => {
    ...
    try {
      const {data} = await useAsyncData('wishlist', async () => {
        const { data } = await useSdk().odoo.query<any, WishlistResponse>({queryName: QueryName.GetWishlist });
        return data.value;
      });
  ...
  const WishlistAddItem = async (id: number) => {
    try {
      loading.value = true;
      const { data }: any = await useSdk().odoo.wishlistAdd(
        { productId: id },
  ...
  const WishlistRemoveItem = async (id: number) => {
    try {
      ...
      const { data }: any = await useSdk().odoo.wishlistRemove(removeItemParams, {
