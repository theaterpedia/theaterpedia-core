...
<template>
  <NuxtLayout name="default" :breadcrumbs="breadcrumbs">
    <NarrowContainer>
      <div class="mb-20 md:px-0" :class="[{ 'px-4': !isRoot }]" data-testid="account-layout">
        <h1
        ...
        <div class="md:flex gap-10" data-testid="account-page-sidebar">
          <div>
            <div
              class="border-t md:border border-neutral-200 pt-4 pb-4 md:p-4 md:rounded-md min-w-[300px] md:block" :class="[
                { hidden: !isRoot },
              ]"
            >
              <ul v-for="{ title, icon, subsections } in sections" :key="title" class="[&:not(:first-child)]:mt-4">
                ...
                <li v-for="{ label, link } in subsections" :key="label">
                  <SfListItem
                    :tag="NuxtLink"
                    :to="link"
                    class="first-of-type:py-4 md:first-of-type:px-4 md:first-of-type:py-2 rounded-md active:bg-primary-100 !text-neutral-900" :class="[
                      {
