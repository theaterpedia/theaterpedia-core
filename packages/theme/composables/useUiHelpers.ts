...
export function useUiHelpers() {
  ...
  const getFacetsFromURL = (query: any): QueryProductsArgs => {
      ...
      Object.keys(query).forEach((filterKey) => {
        if (![...queryParamsNotFilters, 'price'].includes(filterKey)) {
          if (query[filterKey].includes(',')) {
            // #TODO _07 fix linting-err here: label is not used
    ...
    const pageSize = query.itemsPerPage ? Number.parseInt(query.itemsPerPage) : 12
  ...
  const facetsFromUrlToFilter = () => {
    ...
    Object.keys(query).forEach((label) => {
      if (queryParamsNotFilters.includes(label))
        return
